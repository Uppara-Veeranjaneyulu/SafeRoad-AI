...
def build_resnet50_model(input_shape=(224, 224, 3), num_classes=3, dropout_rate=0.4, dense_units=128):
    base_model = ResNet50(weights='imagenet', include_top=False, input_shape=input_shape)
    base_model.trainable = False

    inputs = layers.Input(shape=input_shape)
    # Standard Rescaling [0, 1] to [0, 255] then mean-centering for ImageNet
    x = layers.Rescaling(255.0, offset=-120.0)(inputs)
...
def train_and_evaluate_resnet(head_epochs=6, finetune_epochs=6, batch_size=32):
    ...
    model_path = 'trained_models/resnet50_best.keras'
    cb_list = [
    ...
    print("\n[Training ResNet50] Phase 1: Training Classification Head...")
    ...
    print("\n[Training ResNet50] Phase 2: Fine-tuning Upper Conv Block...")
    ...
    with open('results/resnet50/history.json', 'w') as f:
    ...
    ax1.set_title('ResNet50 - Accuracy Curves')
    ...
    ax2.set_title('ResNet50 - Loss Curves')
    ax2.set_xlabel('Epoch')
    ax2.set_ylabel('Loss')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('results/resnet50/training_curves.png', dpi=300)
    plt.close()

    # Unseen Test Set Evaluation
    best_model = tf.keras.models.load_model(model_path, safe_mode=False)
    print("\n[Evaluation] Evaluating ResNet50 on unseen test set...")
    t0 = time.time()
    ...
    plt.tight_layout()
    plt.savefig('results/resnet50/confusion_matrix.png', dpi=300)
    plt.close()

    report_str = classification_report(test_labels, test_preds, target_names=CLASS_NAMES, digits=4, zero_division=0)
    with open('results/resnet50/classification_report.txt', 'w') as f:
        f.write(report_str)
    ...
    with open('results/resnet50/test_metrics.json', 'w') as f:
