...
def build_efficientnetb0_model(input_shape=(224, 224, 3), num_classes=3, dropout_rate=0.3, dense_units=128):
    # EfficientNetB0 has internal rescaling when using standard input, but since inputs are in [0, 1], scale to [0, 255]
    base_model = EfficientNetB0(weights='imagenet', include_top=False, input_shape=input_shape)
    base_model.trainable = False

    inputs = layers.Input(shape=input_shape)
    x = inputs * 255.0  # EfficientNet expects [0, 255]
...
def train_and_evaluate_efficientnet(head_epochs=6, finetune_epochs=6, batch_size=32):
    os.makedirs('trained_models', exist_ok=True)
    os.makedirs('results/efficientnetb0', exist_ok=True)
    ...
    model_path = 'trained_models/efficientnetb0_best.keras'
    cb_list = [
    ...
    print("\n[Training EfficientNetB0] Phase 1: Training Classification Head...")
    ...
    print("\n[Training EfficientNetB0] Phase 2: Fine-tuning Upper Layers...")
    ...
    with open('results/efficientnetb0/history.json', 'w') as f:
    ...
    ax1.set_title('EfficientNetB0 - Accuracy Curves')
    ...
    ax2.set_title('EfficientNetB0 - Loss Curves')
    ax2.set_xlabel('Epoch')
    ax2.set_ylabel('Loss')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('results/efficientnetb0/training_curves.png', dpi=300)
    plt.close()

    # Unseen Test Set Evaluation
    best_model = tf.keras.models.load_model(model_path)
    print("\n[Evaluation] Evaluating EfficientNetB0 on unseen test set...")
    t0 = time.time()
    ...
    plt.tight_layout()
    plt.savefig('results/efficientnetb0/confusion_matrix.png', dpi=300)
    plt.close()

    report_str = classification_report(test_labels, test_preds, target_names=CLASS_NAMES, digits=4, zero_division=0)
    with open('results/efficientnetb0/classification_report.txt', 'w') as f:
        f.write(report_str)
    ...
    with open('results/efficientnetb0/test_metrics.json', 'w') as f:
